...
| Every competitor product has a real, verified product photo (not just a company logo) | **VERIFIED — 74/74 (100%)** | 4 parallel research agents (2026-08-27) closed the 66-product gap (was Air-only, 8/74). Wired into `CompanyProfileDrawer` and the per-product `ProductFocus` Competitors list. Fixed a real bug in the process: `loading="lazy"` on `<img>` inside the fixed-position drawer overlay silently never fired the network request (confirmed via an isolated `new Image()` test — the same URL loads fine outside that DOM context); removed lazy-loading from both drawer usages. Live-verified 9/9 images across 3 companies post-fix, all real `naturalWidth`. |
...
| Companies and countries are clickable into something real (not just decoration) | VERIFIED | Country cards filter the company list below (verified count-consistent: Germany shows exactly its 5 real HQ'd companies, including the 0-product ones like GARDENA that were previously silently dropped by the count-based list — a real inconsistency caught and fixed). Company cards (Overview standout grid, bottom Companies grid, everywhere) open a real `CompanyProfileDrawer`: logo, HQ, all their real competing products with live price comparisons against the matched Versuni product, click-through into that Versuni product's own drawer. Live-tested: Germany filter, 0-product company (GARDENA, honest empty state), 2-product company (Braun, real price comparisons using the audit-corrected price), nested drawer-to-drawer navigation. Zero console errors across the whole chain. |
